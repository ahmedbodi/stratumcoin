...
The Stratumcoin Core daemon can be configured to act as a trusted "border
router", implementing the stratumcoin wire protocol and relay, making
...
The ZeroMQ feature in Stratumcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building stratumcoind:
...
For instance:
    ...
    $ stratumcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/stratumcoind.tx.raw \
...
These options can also be provided in stratumcoin.conf.
...
From the perspective of stratumcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into stratumcoind directly. Furthermore, no information is
...
using. Stratumcoind appends an up-counting sequence number to each
