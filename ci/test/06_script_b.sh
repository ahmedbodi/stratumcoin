...
if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
  ...
  DOCKER_EXEC ${TEST_RUNNER_ENV} DIR_UNIT_TEST_DATA=${DIR_UNIT_TEST_DATA} LD_LIBRARY_PATH=$DEPENDS_DIR/$HOST/lib "${BASE_BUILD_DIR}/stratumcoin-*/src/test/test_stratumcoin*" --catch_system_errors=no -l test_suite
  END_FOLD
fi
