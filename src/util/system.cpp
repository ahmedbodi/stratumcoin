...
const char * const BITCOIN_CONF_FILENAME = "stratumcoin.conf";
...
#else
    const char* pszModule = "stratumcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Stratumcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Stratumcoin";
#else
    // Unix-like
    return pathRet / ".stratumcoin";
#endif
#endif
}
