    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional STC outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes STC and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::STC_TO_STC) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a STC-to-STC transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::STC_ONLY;
        ...
        // If STC-to-STC fails, create a peg-out transaction
